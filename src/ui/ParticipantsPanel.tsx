  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-3 border-b border-gray-200 dark:border-[#1E1E1E] flex items-center justify-between">
        <h3 className="font-bold text-gray-900 dark:text-white">المشاركون</h3>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {participants.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-gray-400 gap-2">
            ...
            <p className="text-sm text-center">لا يوجد مشاركون بعد</p>
          </div>
        ) : (
              ...
              >
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    {isHost ? (
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-[11px] font-bold text-amber-600 dark:text-amber-400">مضيف</span>
                      </div>
                    ) : (
                      <div className="flex items-center gap-1">
                        <User className="w-3 h-3 text-indigo-400" />
                        <span className="text-[11px] font-bold text-indigo-600 dark:text-indigo-400">ضيف</span>
                      </div>
                    )}
                    <span className="text-[10px] text-gray-400">·</span>
                    <span className="text-[11px] text-gray-500 dark:text-gray-400 truncate">
                      {getLanguageName(participant.language || 'ar')}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  {participant.isScreenSharing && (
                    <div className="w-7 h-7 rounded-xl bg-green-100 dark:bg-green-500/20 flex items-center justify-center" title="يشارك الشاشة">
                  ...
                  }`} title={participant.isCameraOn ? 'الكاميرا شغالة' : 'الكاميرا مغلقة'}>
                    {participant.isCameraOn ? (
                  ...
                  <div className={`w-7 h-7 rounded-xl flex items-center justify-center ${
                    participant.isMicOn
                      ? 'bg-green-100 dark:bg-green-500/20'
                      : 'bg-gray-100 dark:bg-[#2A2A2A]'
                  }`} title={participant.isMicOn ? 'الميكروفون شغال' : 'الميكروفون مكتوم'}>
                    {participant.isMicOn ? (
