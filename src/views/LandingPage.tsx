...
export default function LandingPage({ onCreateRoom, onJoinRoom }: LandingPageProps) {
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] flex flex-col bg-gray-50 dark:bg-[#080808] selection:bg-[#FF4D00] selection:text-white">
      {/* Hero */}
      <section className="relative overflow-hidden pt-20 pb-32">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center flex flex-col items-center">
            {/* Tag */}
            <div className="animate-fade-up inline-flex items-center gap-2 px-5 py-2 rounded-full bg-orange-50 dark:bg-[#1a0800] border border-orange-200 dark:border-[#FF4D00]/20 mb-8">
            ...
            {/* Heading */}
            <h1 className="animate-fade-up delay-100 text-[56px] sm:text-[88px] lg:text-[100px] font-extrabold text-gray-900 dark:text-white tracking-tighter leading-[0.95] max-w-4xl mx-auto">
              اجتماعات بدون
              <br />
            ...
            {/* Subtext */}
            <p className="animate-fade-up delay-200 mt-8 text-lg sm:text-2xl text-gray-500 dark:text-[#A3A3A3] max-w-2xl mx-auto leading-relaxed font-light">
              TalkBridge يجمع مكالمات الفيديو مع الترجمة الصوتية الفورية. تكلم بالعربي المصري والآخر يسمعك بالإنجليزي، في نفس اللحظة.
            </p>

            {/* Features mini */}
            <div className="animate-fade-up delay-300 mt-10 flex flex-wrap items-center justify-center gap-6 text-sm font-bold text-gray-600 dark:text-[#737373]">
              <span className="flex items-center gap-2 bg-white dark:bg-[#121212] px-4 py-2 rounded-full border border-gray-200 dark:border-[#1E1E1E] shadow-sm dark:shadow-none">
            ...
            {/* CTA Actions */}
            <div className="animate-fade-up delay-400 mt-14 flex flex-col md:flex-row items-center justify-center gap-4 w-full max-w-2xl mx-auto">
              
              {/* Action 1: Create New Room */}
              <button
                onClick={handleCreateRoom}
                className="group flex-1 w-full md:w-auto flex items-center justify-center gap-3 px-8 py-5 bg-[#FF4D00] text-white rounded-2xl text-lg font-bold 
              ...
              <form onSubmit={handleJoinRoom} className="flex-1 w-full md:w-auto flex items-center gap-2 relative">
                <input
                  ...
                  dir="rtl"
                  className="w-full px-6 py-5 bg-white dark:bg-[#121212] border border-gray-200 dark:border-gray-800 rounded-2xl text-lg font-bold text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:border-[#FF4D00] focus:ring-1 focus:ring-[#FF4D00] transition-all shadow-sm"
                />
                <button
                  type="submit"
            ...
            {/* How it works */}
            <div className="animate-fade-up delay-500 mt-20 grid sm:grid-cols-4 gap-4 max-w-4xl mx-auto">
              {[
                ...
                <div key={item.step} className="flex items-center">
                  <div className="flex flex-col items-center w-full">
                    <div className="w-16 h-16 rounded-[20px] bg-white dark:bg-[#121212] border border-gray-200 dark:border-[#1E1E1E] flex items-center justify-center mb-4 group-hover:border-[#FF4D00]/50 transition-colors shadow-sm dark:shadow-none">
                      <item.icon className="w-7 h-7 text-gray-400 dark:text-[#A3A3A3]" />
                    </div>
                    <span className="text-sm font-bold text-gray-700 dark:text-[#D9D9D9] tracking-wide">{item.label}</span>
                  </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-32">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" dir="rtl">
          {features.map((feature, i) => (
            <div
              key={feature.title}
              className={`animate-fade-up group p-8 rounded-[35px] bg-white dark:bg-[#121212] border border-gray-200 dark:border-[#1E1E1E]
            ...
            >
              ...
              <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-3 tracking-tight text-right">
                {feature.title}
              </h3>
              <p className="text-base text-gray-500 dark:text-[#737373] leading-relaxed text-right">
                {feature.description}
              </p>
            </div>
          ))}
      ...
      {/* Footer */}
      <footer className="mt-auto border-t border-gray-200 dark:border-[#1E1E1E] py-16 bg-gray-50 dark:bg-[#080808] relative overflow-hidden">
        {/* Glow behind footer */}
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center justify-center gap-6 relative z-10">
