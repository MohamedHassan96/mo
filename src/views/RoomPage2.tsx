            ...
            <div className="space-y-4 sm:space-y-5">
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input type="text" value={customRoomId} onChange={(e) => setCustomRoomId(e.target.value.toLowerCase())} dir="ltr" className="w-full px-4 sm:px-5 py-3.5 sm:py-4 rounded-[18px] sm:rounded-[20px] bg-gray-50 dark:bg-black/40 border border-gray-200 dark:border-white/10 text-gray-950 dark:text-white placeholder-gray-500 dark:placeholder-gray-600 font-mono tracking-widest focus:outline-none focus:border-[#FF4D00] focus:ring-1 focus:ring-[#FF4D00] transition-all lowercase" style={{ paddingLeft: isRtl ? '3.5rem' : '1.25rem', paddingRight: isRtl ? '1.25rem' : '3.5rem' }} />
                  <button
                    type="button"
                    onClick={handleCopyRoomCode}
                    className={`absolute top-1/2 -translate-y-1/2 w-10 h-10 rounded-2xl bg-white dark:bg-[#1A1A1A] border border-gray-200 dark:border-white/10 text-[#FF4D00] flex items-center justify-center hover:border-[#FF4D00]/50 transition-colors ${isRtl ? 'left-2' : 'right-2'}`}
              ...
              <div className="space-y-2">
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder={role === 'host' ? t.hostNamePlaceholder : t.guestNamePlaceholder} dir="auto" className="w-full px-4 sm:px-5 py-3.5 sm:py-4 rounded-[18px] sm:rounded-[20px] bg-gray-50 dark:bg-black/40 border border-gray-200 dark:border-white/10 text-gray-950 dark:text-white placeholder-gray-500 dark:placeholder-gray-600 focus:outline-none focus:border-[#FF4D00] focus:ring-1 focus:ring-[#FF4D00] transition-all" />
              </div>
      ...
      <div className="relative z-10 text-center max-w-md animate-fade-up">
        ...
        {role === 'host' && (
          <div className="mt-8 sm:mt-10 p-4 sm:p-6 bg-white/90 dark:bg-white/5 backdrop-blur-xl rounded-[24px] sm:rounded-[32px] border border-gray-200 dark:border-white/10 shadow-xl dark:shadow-2xl">
            ...
            <div className="bg-gray-50 dark:bg-black/50 rounded-[20px] p-4 mb-4 border border-gray-200 dark:border-white/5 cursor-pointer hover:border-[#FF4D00]/50 transition-colors" onClick={handleCopyLink} title={t.copyRoomLinkBtn}>
              <p className="text-xs font-mono text-[#FF4D00] break-all text-left" dir="ltr">{getInviteLink()}</p>
            </div>
      ...
      <div className="bg-white/85 dark:bg-white/5 backdrop-blur-2xl border-b border-gray-200 dark:border-white/10 px-3 sm:px-6 py-3 sm:py-4 flex flex-col sm:flex-row items-stretch sm:items-center justify-between z-40 shadow-sm relative gap-3 sm:gap-0">
        ...
        <div className="flex items-center gap-4 sm:mt-0">
          <div className="w-full sm:w-auto flex items-center justify-center gap-2 sm:gap-3 bg-gray-100 dark:bg-white/5 px-3 sm:px-5 py-2.5 rounded-2xl border border-gray-200 dark:border-white/10 shadow-inner">
            ...
            <ArrowRight className={`w-4 h-4 text-[#FF4D00] ${isRtl ? 'scale-x-[-1]' : ''}`} />
